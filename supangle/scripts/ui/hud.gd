extends Control
## Oyun içi HUD: can barı, kalkan göstergesi, canavar sayacı ve güç durumu.

const COLOR_READY := Color(0.98, 0.85, 0.35, 1.0)
const COLOR_ACTIVE := Color(1.0, 1.0, 1.0, 1.0)
const COLOR_RECHARGING := Color(1.0, 1.0, 1.0, 0.35)
...
func _ready() -> void:
	...
	if player != null:
		...
		_on_shield_state_changed(player.shield_ready)
	_refresh_powers()
...
func _on_shield_state_changed(is_ready: bool) -> void:
	shield_label.modulate = COLOR_READY if is_ready else COLOR_RECHARGING
	shield_label.text = "Kalkan: Hazır" if is_ready else "Kalkan: Doluyor..."

func _refresh_powers() -> void:
	power_immortality.modulate = COLOR_ACTIVE if GameState.has_power(GameState.Power.IMMORTALITY) else COLOR_LOST
