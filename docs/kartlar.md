...
| XP taşı değeri | temel 1 (mavi), hızlı 2 (turuncu), tank 7 (mor) | `enemy*.tscn` → `xp_value` |
| XP taşı rengi | değere göre: 5+ mor, 2+ turuncu, altı mavi | `xp_gem.gd` → `GEM_TIERS` |
...
| Spawn temposu | başlangıç 1.2 / 0.9 / 1.05 sn, her spawnda 0.022 sn kısalır, taban 0.28 sn | `enemy_spawner.gd`, `level*.tscn` |
| Aynı anda azami canavar | 150 | `enemy_spawner.gd` → `max_enemies` |
| Boss canları | 175 / 450 / 480 | `boss*.tscn` → `max_health` |
